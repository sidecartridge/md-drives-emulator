...
- **Command answers no longer wait for the activity LED**: the RP updated the LED after running each command but before writing the answer the ST polls for. On a Pico W the LED sits on the Wi-Fi chip, so each update is a bus transaction. The answer now goes first.
