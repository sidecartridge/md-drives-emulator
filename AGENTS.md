...
- The floppy emulation sits in the disk vectors, `hdv_bpb`, `hdv_rw` and `hdv_mediach` (`floppy.s`, `floppy_hdv_*`, each behind an XBRA `SDFE` header whose old vector the RP writes), where every TOS disk driver sits - never at the BIOS trap in front of them. Whatever wraps those vectors has to see the emulated drives: the TOS desktop's Esc on a window (`aes/trap14.S`, `_mediach`) wraps all three to force a media change on the window's drive and counts on GEMDOS's `Getbpb` for that drive going through its wrapper to put them back. Answered at the trap, it never did, the wrappers stayed, and the next Esc wrapped them around themselves: every disk call behind them - the computer's own drive, ACSI - then looped for ever.
- Except while the disk's boot sector runs. `floppy_start` hooks the BIOS trap first (`bios_trap`, which sends `Getbpb`, `Mediach` and `Rwabs` for an emulated drive into the same handlers), leaves the three vectors as TOS set them, runs the boot sector, and only then moves into the vectors (`set_vectors_hdv`), taking the trap hook out if `$B4` still points at it. On a real ST a boot sector runs before any hard-disk driver, so it sees the ROM's vectors, and menu disks check that: Medway's boot sector says "A virus is lurking in memory" unless the high word of `hdv_bpb` is `$00FC`. A boot sector that never returns leaves the trap hook in; one that hooks the BIOS after it does too, and then the desktop's Esc rule above does not hold for that session.
- Floppy media change follows TOS's own floppy driver (th-otto/tos1x `bios/blkdev.c`, `bdos/fsfat.c`, `bdos/sup.c`): the RP raises `MED_CHANGED` when drive A's slot is cycled, and when a write reaches a drive's image sector 0 - after rebuilding that drive's BPB in the window from the new boot sector, before it answers - as TOS's `flopwrt` marks a boot-sector write a change and its `getbpb` rereads the boot sector each time; `Mediach` answers it; `Rwabs` in modes 0 and 1 answers `E_CHNG` without a transfer while it is set; GEMDOS then logs the drive out, retries and calls `Getbpb`, and `floppy.s`'s `Getbpb` ends the change by telling the RP (`set_media_change`, `CMD_SET_SHARED_VAR`), as TOS's `getbpb` resets its own state. `Rwabs` with a NULL buffer sets the state from its count, as TOS does. The ST never writes the window; it sends the command.
- The old rule - clear the flag when the new disk's root-directory sector is read, and keep `floppy.s` read-only - was replaced on 2026-09-24. It left the flag stuck when the root sector could not be computed from a malformed BPB, and let GEMDOS loop on `E_CHNG` through a call that re-read a cached FAT sector first. The earlier note that clearing from the ST "caused instability" had no record in git of what was tried; the send now happens only in `Getbpb` and `Rwabs`, the same trap context `Rwabs` has always sent from.
...
- Trap #1 hooks must not push anything on the caller's stack before they know they handle the call. TOS 1.04 starts GEM (`aes/gemjstrt.S` in the TOS source reconstruction) on a 132-byte stack (`ustak`), with `Super()` putting the supervisor stack there too, and calls GEMDOS from it (Super, Mshrink, Malloc) with about 110 bytes left above the AES's resolution variables (`gl_rschange`, `gl_restype`) and then GEMDOS globals. GEMDRIVE used to save 48 bytes of registers before its dispatch, and the pool fix 60 more plus `send_sync`: on some boots GEM's standard handles were zeroed, so every TOS program's console output went to MIDI (handle 0 is BIOS device 3 on TOS 1.x), and the same overflow can reach the resolution variables (one suspected cause of GEM starting in low resolution despite `DESKTOP.INF`; a remaining case is still open). GEMDRIVE looks the opcode up with `d0` alone (and `a0` for a user-mode caller), and a call it takes runs on its own 1 KB stack (`Malloc`ed at start, owned by the initial process; top in its shared variable 22, `SHARED_VARIABLE_STACK`), which costs the caller's 4 bytes (`a6`); the pool fix switches to its own 1 KB stack the same way (top in `pf_stack`). Both matter: at boot a GEMDOS call nested in the first call GEMDRIVE takes (TOS's AUTO-folder `Fsetdta`, whose `Dgetdrv` TOS saves 50 bytes of context for on the caller's stack, `systk`) had less than 8 bytes to spare - 8 more bytes of saved registers there made that call return to TOS's boot code instead of to GEMDRIVE, left the re-entry lock set, and GEMDRIVE took no call after that.
- A GEMDOS hook gives the caller its registers back as TOS's GEMDOS does: every one but `d0` to a caller in supervisor mode, all but `a0` to one in user mode (measured on TOS 1.04 and 2.06 from a floppy, with no drive emulation in front of GEMDOS). Callers count on it: the desktop's Esc, in supervisor mode, sets `a0` to 0 before two `trap #1` calls and reads `_hdv_bpb(a0)` after them to take its wrappers out of the disk vectors. So the lookup that decides whether a call is ours uses only `d0` (and `a0` in user mode), a call that is not ours goes on with every register as it came, and a call that is ours saves d1-d7/a0-a5 on the hook's own stack. FSTESTS checks it (the `registers` case).
...
- Never remove an entry from `defaultEntries` in `rp/src/aconfig.c`. `settingsLoadAllEntries` (`rp/src/settings/settings.c`) reads only as many stored entries as there are defaults, in the order they were stored, matched by key; with one default fewer, the last setting a card had saved is not read and falls back to its default (the pool fix switch, as the list stands). Retire a setting by leaving its entry and no longer reading it, as `ACSI_ID` is. The loader's bound is also what keeps it from reading the unused tail of the region, which `settings_save` writes from an uninitialised buffer, so it cannot simply read to the end.
...
- While the host has the card mounted over USB MSC, the setup menu does not start emulation: `[E]` - typed, or sent with `swd.py key e` - does nothing, and the boot countdown is halted (`usbMassStorageReady` in `emul.c`). Eject the card on the host first (`diskutil eject /Volumes/IMG`, then check it is gone).
- While the Mac's screen is locked, macOS will not mount the card: `loginwindow` dissents the mount approval and ejects the disk (`/usr/bin/log show --predicate 'process == "diskarbitrationd"'` shows `disk mount approval ... dissented`), and the RP console shows `Inquiry` then `UNLOAD DISK STORAGE` a second later. Nothing is wrong with the device; the card mounts once the Mac is unlocked and the RP re-enumerates (flash or `swd.py reset`). The screen locks when the display sleeps, a few minutes into a quiet run, so keep it awake for a hardware session: `caffeinate -d -i -u -t 10800` in the background.
- A hardware session runs without hands between runs. Every program put on the ST for a run - FSTESTS, FLOPTEST, any one-off probe - ends by writing its log and then restarting the device and rebooting the computer: `sidecart_restart_device()` (`tests/atarist/src/sidecart.c`, command `$048C`) and a jump through the reset vector, as `end_of_run()` in `tests/atarist/src/test_runner.c` does. The device comes back in the setup menu with the card on USB, and the ST reboots into that menu, ready for the next run. `swd.py reset` restarts the RP only: a running ST is left hanging and needs its reset button, so use it only when nothing of ours runs on the ST. To see what is on the setup screen, snapshot it instead of asking: `swd.py screen <file.png>` renders the framebuffer the RP draws for the ST (the setup menu, not GEM or a program), `swd.py text` prints its text.
