...
- Floppy media change follows TOS's own floppy driver (th-otto/tos1x `bios/blkdev.c`, `bdos/fsfat.c`, `bdos/sup.c`): the RP raises `MED_CHANGED` when drive A's slot is cycled; `Mediach` answers it; `Rwabs` in modes 0 and 1 answers `E_CHNG` without a transfer while it is set; GEMDOS then logs the drive out, retries and calls `Getbpb`, and `floppy.s`'s `Getbpb` ends the change by telling the RP (`set_media_change`, `CMD_SET_SHARED_VAR`), as TOS's `getbpb` resets its own state. `Rwabs` with a NULL buffer sets the state from its count, as TOS does. The ST never writes the window; it sends the command.
